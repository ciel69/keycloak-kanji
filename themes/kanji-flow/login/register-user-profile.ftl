  ...
  <form class="space-y-4" action="${url.registrationAction}" method="post">
    ...
    <#list profile.attributes as attribute>
      <#if attribute.name == "username" || attribute.name == "email">
        <div>
          <label for="${attribute.name}" class="block text-sm font-medium text-muted">
          ...
          <input id="${attribute.name}" name="${attribute.name}"
                 ...
                 class="mt-1 block w-full rounded-md shadow-sm focus:ring-2 focus:ring-plum-500 focus:outline-none transition-colors px-3 py-2 text-sm border bg-surface text-foreground border-border focus:border-plum-500 <#if messagesPerField.existsError(attribute.name)>border-red-500 dark:border-red-500</#if>"
    ...
    <div>
      ...
      <div class="relative mt-1">
        <input id="password" name="password" type="password" required autocomplete="new-password"
               class="block w-full rounded-md shadow-sm focus:ring-2 focus:ring-plum-500 focus:outline-none transition-colors px-3 py-2 pr-10 text-sm border bg-surface text-foreground border-border focus:border-plum-500 <#if messagesPerField.existsError('password')>border-red-500 dark:border-red-500</#if>"
